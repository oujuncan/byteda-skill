...
- `generate` 默认走 SSE 流式，实时把进度（`[10%]…[95%]`）打到 stderr，通常耗时 1-3 分钟（复杂任务更久）；加 `--no-stream` 可回退到单次同步请求。请配置足够长的超时（脚本默认 1200s，`--timeout 0` 不限）。断流时 CLI 会区分来源：`[客户端断开]`（本地 `--timeout` 到期，可加大超时）vs `[服务端断开]`（上游/服务端提前关流，任务可能仍在服务端继续，可重试或用 `--no-stream`）。
...
- 默认 `--timeout` 从 300s 提高到 1200s，长/复杂生成不再默认被客户端提前掐断。
- SSE 流中断时，CLI 会区分断开来源：`[客户端断开]`（本地 `--timeout` 到期，服务端任务可能仍在继续）
  vs `[服务端断开]`（上游/服务端提前关流，含优雅 EOF 与连接重置），并在提示里给出重试或
  `--no-stream` 建议。
